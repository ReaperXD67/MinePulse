...
async function main() {
  ...
  try {
    ...
    assert(!registrationBody.user || registrationBody.user.role === "PLAYER", "Public signup was able to choose a privileged role");
    ...
    if (registrationBody.requiresEmailVerification) {
      assert(
        !state.cookies.some((entry) => entry.name === "karixmc_session"),
        "Unverified registration received an authenticated session"
      );
      await prisma.user.update({ where: { id: userId }, data: { emailVerifiedAt: new Date() } });
      const verifiedLogin = await first.post("/api/auth/login", { data: { email, password } });
      assert(verifiedLogin.ok(), `Verified login failed with ${verifiedLogin.status()}: ${JSON.stringify(await json(verifiedLogin))}`);
    ...
    const authenticatedSessions = await first.get("/api/account/sessions");
    assert(
      authenticatedSessions.ok(),
      `Authenticated session endpoint is unavailable (${authenticatedSessions.status()}), cookie=${JSON.stringify(cookie)}`
    );
